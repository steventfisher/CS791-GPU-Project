...
/****************************Function Prototypes*******************************/
void convertAdjListToArray(Graph * G, int * vertexArray, int * edgeArray, int * distArray);
void multipleGPUThreadFunction(int * vertexArray, int * edgeArray, int * distArray, int numVertices, int numEdges, int * source, int deviceNum, int ** gpuCost, int numIngress, int numDevices);



/******************************CUDA Kernels************************************/
__global__ void bfs_kernel(int * vertexArray, int * edgeArray, int * distArray, bool * frontierArray, bool * frontierUpdatingArray, bool * visitedArray, int * costArray, int numVertices )
{
	// get the thread id
    ...
    {                   
        ...
        {                       
            ...
            {
				if(distArray[destVertex] != 1){
					frontierUpdatingArray[destVertex] = false;
					costArray[destVertex] = costArray[tid] + 1;
					frontierArray[tid] = true;
					distArray[destVertex] -= 1;
				}
				else{
					// increment the cost
					costArray[destVertex] = costArray[tid] + 1;
...
float processGraphSingleGPU(Graph * graph, int * source, int numIngress, int ** gpuCost)
    {
        ...
        int * distArray = new int [numEdges];

        // start GPU timer
        cudaEventRecord( start, 0 );

        convertAdjListToArray(graph, vertexArray, edgeArray, distArray);


   
		multipleGPUThreadFunction(vertexArray, edgeArray, distArray, numVertices, numEdges, source, 0, gpuCost, numIngress, 1);
...
float processGraphMultipleGPU(Graph * graph, int * source, int numIngress, int ** gpuCost, int numDevices)
    {
		...
		while( i < numDevices )
		{
				...
				// launch one thread
				threadList[i] = std::thread(multipleGPUThreadFunction, vertexArray, edgeArray, distArray, numVertices, numEdges, source, i, gpuCost, numIngress, numDevices);
...
void multipleGPUThreadFunction(int * vertexArray, int * edgeArray, int * distArray, int numVertices, int numEdges, int * source, int deviceNum, int ** gpuCost, int numIngress, int numDevices)
{
        ...
        for(int n = deviceNum; n < numIngress; n += numDevices )
        {
            ...
            // create the cuda memory and send to the device 
                ...
                // edge array
                int* deviceDistArray;
                cudaMalloc( (void**) &deviceDistArray, sizeof(int)*numEdges);
                cudaMemcpy( deviceDistArray, distArray, sizeof(int)*numEdges, cudaMemcpyHostToDevice);
            ...
            while(searching)
            {
                ...
                // start BFS kernel
                bfs_kernel<<<numBlocks, numThreads>>>(deviceVertexArray, deviceEdgeArray, deviceDistArray, deviceFrontierArray, deviceFrontierUpdatingArray, deviceVisitedArray, deviceCostArray, numVertices);
...
void convertAdjListToArray(Graph * G, int * vertexArray, int * edgeArray, int * distArray)
    {
        ...
        for (int i = 0; i < G->graphVector.size(); i++)
        {
            ...
            for (auto it : G->graphVector[i].edges)
            {
                ...
                distArray[edgeCount] = std::get<1>(it);
